...
- **Corner trips**: a pack that ships a `pee` animation will occasionally
  trot to the nearest edge of whatever it is standing on, turn its tail to
  the wall and mark the spot. Packs without that art never do it, so none
  of the bundled characters are affected.
...
- **Menu**: right-click the cat for settings / window-hop / walk / nap /
  mute / hide (plus "Find a corner" for packs with `pee` art).
...
omarchy-shell omate pee            # trot to the nearest corner (packs
                                   # with "pee" art only)
